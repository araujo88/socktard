...
unsigned long int WinSocket::receiveData(SOCKET client_socket, std::string *data)
{
    unsigned long int bytes = recv(client_socket, (*data).c_str(), (*data).size(), 0);
    if (data < 0)
    {
...
void WinSocket::sendData(SOCKET client_socket, const std::string bytes)
{
